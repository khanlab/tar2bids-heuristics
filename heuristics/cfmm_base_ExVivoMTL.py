...
def infotodict(seqinfo):
    ...
    #FLASH:
    t1_flash = create_key('{bids_subject_session_dir}/anat/{bids_subject_session_prefix}_acq-T1w_run-{item:02d}_FLASH')
    
    #CISS T2w:
    t2_ciss = create_key('{bids_subject_session_dir}/anat/{bids_subject_session_prefix}_acq-CISS_run-{item:02d}_T2w')
    ...
    #tse tra T2w
        ...
        #spc T2w
        if ('spc_flair' in s.series_description or 't2_space_dark-fluid' in s.series_description ): 
            if ('ND' in (s.image_type[3].strip())):
        ...
        if ('t1_fl3d' in s.series_description ): 
            info[t1_flash].append({'item': s.series_id})
            
        if ('ciss3d' in s.series_description ): 
            info[t2_ciss].append({'item': s.series_id})
