    ...
    for idx, s in enumerate(seqinfo):
        ...
        if ('foodflanker' in (s.series_description).strip()):
            if (s.dim4==1 and 'SBRef' in (s.series_description).strip()):
                info[foodflanker_sbref].append({'item': s.series_id})
            elif (s.dim4>2):
                info[foodflanker].append({'item': s.series_id})

        if ('RSAfood' in (s.series_description).strip()):
            if (s.dim4==1 and 'SBRef' in (s.series_description).strip()):
                info[rsafood_sbref].append({'item': s.series_id})
            elif (s.dim4>2):
                info[rsafood].append({'item': s.series_id})
